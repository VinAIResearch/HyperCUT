...
def unnormalize_tensor(tensor, max_pixel_value=255.):
...
def unnormalize_vid_tensor(vid_tensor, max_pixel_value=255.):
...
def get_visualization(task, pred, dataPoint):
    if task == 'classification':
        ...
        gt_keypoints = [(int(gt_keypoints[2 * i].item()), int(gt_keypoints[2 * i + 1].item())) for i in range(0, gt_keypoints.shape[0] // 2)]
        pred_keypoints = [(int(pred_keypoints[2 * i].item()), int(pred_keypoints[2 * i + 1].item())) for i in range(0, pred_keypoints.shape[0] // 2)]

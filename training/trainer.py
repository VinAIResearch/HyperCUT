...
import logging
import torch
from resources.consts import TRAINING_STATE_SAVE_PATH_FORMAT
from resources.consts import MODEL_SAVE_PATH_FORMAT
from resources.consts import CHECKPOINT_PATH
from resources.consts import VAL_SAMPLE_SAVE_PATH, TRAIN_SAMPLE_SAVE_PATH
...
class Trainer:
    def __init__(self, args):
        ...
        if args.dataset_name in ['b-aist++']:
            train_set, val_set, tral_set, _ = get_datasets_b(
            **vars(args)
        )
        else:
            train_set, val_set, tral_set, _ = get_datasets(
                **vars(args)
            )
        ...
        self.train_dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, data_point in enumerate(self.train_dataloader):
            ...
            if batch_idx % self.args.train_step == 0:
                self.logger.info(f'step: {batch_idx}/{self.epoch_length} lr: {self.get_lr():.2e}, loss: {loss.item():.4f}')
            self.current_iter += 1
    ...
    def save(self):
        ...
        training_state_save_path = \
            TRAINING_STATE_SAVE_PATH_FORMAT.format(self.exp_name, save_name)
    ...
    def validation(self):
        ...
        train_scores = model.validation(self.tral_dataloader, self.args.display_step, limit=500, save_root=train_save_root)

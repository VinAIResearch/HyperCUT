...
from utils.common_utils import visualize_tsne
...
class HyperCUT(nn.Module):
    ...
    def calc_loss(self, out, data, task):
        ...
        diff_loss = 1 / 3. * (diff_loss1 + diff_loss2 + diff_loss3)
        same_loss = 1 / 3. * (same_loss1 + same_loss2 + same_loss3)
    ...
    def test(self, dataloader):
        ...
        with torch.no_grad():
            for data in tqdm(dataloader):
                ...
                d0_np = d00[0].detach().cpu().numpy()
                d1_np = d01[0].detach().cpu().numpy()
                ...
                d0_np = d10[0].detach().cpu().numpy()
                d1_np = d11[0].detach().cpu().numpy()
                ...
                d0_np = d20[0].detach().cpu().numpy()
                d1_np = d21[0].detach().cpu().numpy()

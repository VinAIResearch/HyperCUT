import torch
import models.backbones.cupy_module.adacof as adacof
import sys
import numpy as np
from torch.nn import functional as F
import torch.nn as nn
from utils.training_utils import CharbonnierFunc
from models.backbones.RAFT.raft import RAFT
from utils.training_utils import tensor2img, unnormalize_tensor
from utils.flow_utils import visualize_flow


class CDFI_deblur(torch.nn.Module):
    def __init__(self, args):
        super(CDFI_deblur, self).__init__()
        self.args = args
        ...
        self.optical_flow = nn.DataParallel(RAFT(args))
        self.optical_flow.load_state_dict(torch.load('models/RAFT/pretrained/raft-things.pth'))
        self.optical_flow = self.optical_flow.module
        self.optical_flow.eval()
        for v in self.optical_flow.parameters():
            v.requires_grad = False

    def forward(self, data_in):
        blur = data_in['blur_img']
        mid = data_in['mid_img']
        h0 = int(list(blur.size())[2])
        w0 = int(list(blur.size())[3])
        h2 = int(list(mid.size())[2])
        w2 = int(list(mid.size())[3])
        if h0 != h2 or w0 != w2:
        ...
        if h0 % 32 != 0:
            pad_h = 32 - (h0 % 32)
            blur = F.pad(blur, [0, 0, 0, pad_h], mode='reflect')
            mid = F.pad(mid, [0, 0, 0, pad_h], mode='reflect')
            h_padded = True

        if w0 % 32 != 0:
            pad_w = 32 - (w0 % 32)
            blur = F.pad(blur, [0, pad_w, 0, 0], mode='reflect')
            mid = F.pad(mid, [0, pad_w, 0, 0], mode='reflect')
            w_padded = True
        Weight1, Alpha1, Beta1, Weight2, Alpha2, Beta2, Occlusion, Blend, featConv1, featConv2, featConv3, featConv4, featConv5 \
            = self.get_kernel(blur, mid)

        tensorAdaCoF1 = self.moduleAdaCoF(self.modulePad(blur), Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorAdaCoF2 = self.moduleAdaCoF(self.modulePad(mid), Weight2, Alpha2, Beta2, self.dilation) * 1.

        w, h = self.modulePad(blur).shape[2:]
        ...
        _, gt_flow = self.optical_flow(
            unnormalize_tensor(data_in['mid']),
            unnormalize_tensor(data_in['left']),
            iters=20,
            test_mode=True
        )

        _, pred_flow = self.optical_flow(
            unnormalize_tensor(data_in['mid']),
            unnormalize_tensor(frame1),
            iters=20,
            test_mode=True
        )

        flow_reg = CharbonnierFunc(gt_flow - pred_flow)

        return {'pred': frame1, 'regs': {'GSpatialReg': g_Spatial, 'flowReg': flow_reg}}

    def get_visualizations(self, data_point, pred):
        with torch.no_grad():
            _, gt_flow = self.optical_flow(
                unnormalize_tensor(data_point['input']['mid'][0:1, :, :, :].cuda()),
                unnormalize_tensor(data_point['input']['left'][0:1, :, :, :].cuda()),
                iters=20,
                test_mode=True
            )
            ...
            _, pred_flow = self.optical_flow(
                unnormalize_tensor(data_point['input']['mid'][0:1, :, :, :].cuda()),
                unnormalize_tensor(pred[0:1, :, :, :].cuda()),
                iters=20,
                test_mode=True
            )

        mid_vis = tensor2img(data_point['input']['mid'][0])
        left_vis = tensor2img(data_point['gt'][0])
        gt_flow_vis = visualize_flow(mid_vis, gt_flow[0])

        pred_vis = tensor2img(pred[0])
        pred_flow_vis = visualize_flow(mid_vis, pred_flow[0])

        blur = tensor2img(data_point['input']['blur'][0])

        visualizations = {
            'flows': np.hstack((gt_flow_vis, pred_flow_vis)),
            'left': left_vis,
            'mid': mid_vis,
            'blur': blur,
            'pred': pred_vis,

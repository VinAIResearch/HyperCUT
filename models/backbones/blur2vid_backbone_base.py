...
class OrderInvariantLoss(nn.Module):
    ...
    def forward(self, x1, x2, y1, y2):
        acc = {"hit": 0, "con@2": 0, "con@3": 0}
        return {
            "loss": self.mse(torch.abs(x1 + x2), torch.abs(y1 + y2))
            + self.mse(torch.abs(x1 - x2), torch.abs(y1 - y2)),
            "acc": acc,
        }
...
class OrderInvariantBasedLoss(nn.Module):
    ...
    def forward(self, out, data, task):
        ...
        return {"total": order_inv_loss, "acc": acc}
...
class HyperCUTBasedLoss(nn.Module):
    def __init__(self, epsilon=0.001, **loss_kwargs):
        ...
        self.hypercut.load_state_dict(torch.load(loss_kwargs["HyperCUT"]["pretrained_path"]))
    ...
    def calc_hypercut_loss(self, x, task):
        ...
        loss = (
            self.softplus(self.hypercut.hyperplane(d00))
        ...
        acc = {"hit": hits, "con@2": consist, "con@3": twice_consist}

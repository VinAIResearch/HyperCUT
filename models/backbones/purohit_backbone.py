...
class Purohit(Blur2VidBackboneBase):
    ...
    def forward(self, data):
        ...
        return {"recon_frames": recon_frames[0], "multiscale_recon_frames": recon_frames, "pred_flows": pred_flows}
...
class MultiscaleOrderInvLoss(OrderInvariantBasedLoss):
    ...
    def forward(self, out, data, task):
        ...
        for scale in range(num_scales):
            ...
            gt = torch.cat(
                [F.interpolate(gt[:, i, :, :, :], scale_factor=0.5).unsqueeze(1) for i in range(num_frames)], dim=1
            )
        ...
        loss = {"order_inv_loss": order_inv_loss.item(), "tv_reg": tv_reg.item(), "total": total, "acc": acc}
...
class MultiscaleCharbonnierLoss(NaiveBasedLoss):
    ...
    def forward(self, out, data, task):
        ...
        loss = {"char_loss": char_loss.item(), "tv_reg": tv_reg.item(), "total": total}
...
class MultiscaleHyperCUTLoss(HyperCUTBasedLoss):
    ...
    def forward(self, out, data, task):
        ...
        loss = {"char_loss": order_inv_loss.item(), "tv_reg": tv_reg.item(), "total": total, "acc": acc}

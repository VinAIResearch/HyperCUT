...
class BaistDataset(Dataset):
    def __init__(self, metadata_root, transforms, partition, target_frames=None, limit=None):
        ...
        with open(metadata_root, 'r') as f:
            ...
            for data in metadata['data']:
                ...
                self.blur_anno.append(
                    osp.join(DATASET_ROOT, self.dataset_name, data['bbox'])
                )
    ...
    def __getitem__(self, idx):
        ...
        data = {f'frame{i + 1}': self.transforms['cropbbox'](image=imgs[i], prev_img=imgs[i - 1], bbox=blur_anno) for i in range(len(imgs))}
